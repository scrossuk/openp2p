...
namespace OpenP2P {
	...
	class RPCProtocol : boost::noncopyable {
		...
		public:
			...
			bool sendReply(const IdType& rpcId, const EndpointType& endpoint, const Buffer& data) {
				Lock lock(mutex_);
				if(closed_) return false;
			...
			bool executeGroup(const std::vector< std::pair<EndpointType, Buffer> >& requestList, std::vector< boost::optional<Buffer> >& replyList) {
				Condition condition;
			...
			bool executeFixedSizeGroup(const boost::array<std::pair<EndpointType, Buffer>, N>& requestList, boost::array<boost::optional<Buffer>, N>& replyList) {
				Condition condition;
			...
			void cancel() {
				...
				for(typename std::map<IdType, Listener>::iterator i = rpcs_.begin(); i != rpcs_.end(); ++i) {
					((i->second).condition)->notify();
				}

				receiveCondition_.notify();
			}
